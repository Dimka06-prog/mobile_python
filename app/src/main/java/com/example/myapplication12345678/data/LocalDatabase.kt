...
class LocalDatabase(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    private fun insertInitialData(db: SQLiteDatabase) {
        ...
        // Дополнительные данные: тестовые вопросы
        try {
            val testQuestions = listOf(
                Triple(1, "Что выведет print(2 ** 3)?", listOf("6", "8", "9", "23")) to 2,
                Triple(1, "Какой тип данных у значения True?", listOf("str", "bool", "int", "float")) to 2,
                Triple(2, "Какой оператор используется для ветвления?", listOf("for", "if", "def", "import")) to 2,
                Triple(3, "Как получить длину списка lst?", listOf("size(lst)", "len(lst)", "lst.length", "count(lst)")) to 2
            )

            testQuestions.forEach { (q, correct) ->
                val (courseId, text, opts) = q
                db.execSQL(
                    "INSERT INTO test_questions (course_id, question_text, option1, option2, option3, option4, correct_option) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    arrayOf(courseId, text, opts[0], opts[1], opts[2], opts[3], correct)
                )
            }

            // Дополнительные данные: примеры сниппетов (для демонстрации блокнота)
            db.execSQL(
                "INSERT INTO snippets (user_login, title, code, language, tags) VALUES ('demo', 'Hello World', 'print(\"Hello, ProPython!\")', 'python', 'start,print')"
            )
            db.execSQL(
                "INSERT INTO snippets (user_login, title, code, language, tags) VALUES ('demo', 'Список и цикл', 'nums = [1, 2, 3]\nfor n in nums:\n    print(n)', 'python', 'list,for')"
            )
        } catch (_: Exception) {
            // Игнорируем ошибки seed-данных, чтобы приложение не падало при старте
        }
    }
    ...
    fun addSnippet(
        ...
        return writableDatabase.insert("snippets", null, values)
    }

    fun updateSnippet(id: Int, title: String, code: String, language: String, tags: String): Int {
        val values = ContentValues().apply {
            put("title", title)
            put("code", code)
            put("language", language)
        ...
        return writableDatabase.update("snippets", values, "id = ?", arrayOf(id.toString()))
    }

    fun deleteSnippet(id: Int): Int {
        return writableDatabase.delete("snippets", "id = ?", arrayOf(id.toString()))
    }

    fun searchSnippets(userLogin: String, query: String): List<CodeSnippet> {
        val result = mutableListOf<CodeSnippet>()
        val like = "%${query.trim()}%"
        val cursor = if (query.isBlank()) {
            readableDatabase.rawQuery(
                "SELECT id, user_login, title, code, language, tags, created_at, updated_at FROM snippets WHERE user_login = ? ORDER BY updated_at DESC",
                arrayOf(userLogin)
            )
        } else {
            readableDatabase.rawQuery(
                """
                SELECT id, user_login, title, code, language, tags, created_at, updated_at
                FROM snippets
                WHERE user_login = ? AND (title LIKE ? OR code LIKE ? OR tags LIKE ?)
                ORDER BY updated_at DESC
                """.trimIndent(),
                arrayOf(userLogin, like, like, like)
            )
        }
    ...
    companion object {
        private const val DB_NAME = "propython_local.db"
